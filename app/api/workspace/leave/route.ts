...
export async function POST(request: Request) {
  ...
  try {
    ...
    await database.savedMindMaps.deleteMany({
      where: {
        id: { in: savedMindMapsIds },
      },
    });

    await database.savedTask.deleteMany({
      where: {
        id: { in: savedTaskIds },
      },
    });

    await database.assignedToTask.deleteMany({
      where: {
        id: { in: assignedToTaskIds },
      },
    });

    await database.assignedToMindMap.deleteMany({
      where: {
